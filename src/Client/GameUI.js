...
class GameUI {
    ...
    myTurn() {
        return this.gameInfo.player === this.gameState.turnOf;
    }
     ...
     */
    findCharacter(pos) {
        return [...this.characters.values()].find((c) =>
            c.positionDto.Equals(pos)
        );
    }
    ...
    showActionOrCharacterInfo(pos, preview) {
        ...
        if (preview && preview.results.has(characterBeforeAction.id)) {
            ...
            const characterAfterAction = preview.results.get(
                characterBeforeAction.id
            );
            sendActionMsg(
                actionName,
                characterBeforeAction.model,
     ...
     */
    constructor(gameInfo, gameState, characters, engineInfo) {
        ...
        this.cursor = new Cursor(boardPosToVec3(0, 0), this.engineInfo.scene);
    ...
    static async create() {
        ...
        const tiledGround = MeshBuilder.CreateTiledGround("board", {
            xmin: -0.5,
            zmin: 0.5 - boardInfo.col,
            xmax: -0.5 + boardInfo.row,
            zmax: 0.5,
        ...
        const tileIndicesLength =
            tiledGround.getIndices().length / (boardInfo.row * boardInfo.col);
        ...
        for (let col = boardInfo.col - 1; col >= 0; col--) {
            for (let row = boardInfo.row - 1; row >= 0; row--) {
                ...
                const mountainMesh =
                    mountainMeshes[
                        Math.floor(Math.random() * mountainMeshes.length)
                    ];
                SceneLoader.ImportMesh(
                    ...
                    scene,
                    (newMeshes) => {
                        const modelRootMesh = new AbstractMesh(
                            `root-mountain-${row}-${col}`
                        );
                        newMeshes.forEach((m) => {
                            m.setParent(modelRootMesh);
                        });
                        modelRootMesh.scaling = new Vector3(1.35, 1.35, 1.35);
                        modelRootMesh.rotation.y = Tools.ToRadians(
                            Math.random() * 360
                        );
        ...
        let differentPlayer;
        if (gameInfo.player === Player.Player1)
            differentPlayer = Player.Player2;
